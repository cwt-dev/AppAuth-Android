...
public final class BrowserSelector {
     ...
     *
     * Workaround for Android Jetifier bug
     * -----------------------------------
     * Problem: Projects that are using androidx are using a Jetifier compiler as a dependency. The Jetifier
     * compiler is replacing "android.support" with "androidx" inside all the dependencies, AppAuth included.
     * But "android.support" is still required by the platform in order to open a chrome tab. As a result -
     * a chrome tab is not found, and an external browser is opened instead.
     * Workaround: Using StringBuilder to create the string "android.support.customtabs.action.CustomTabsService"
     * in runtime prevents the jetifier static regex from matching "android.support" in the compiled class,
     * thus avoiding the androidx replacement.
     */
